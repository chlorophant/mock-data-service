...
import { persons as table } from '../../db/schema'
import { get_config, get_by_id_config, post_config, patch_config, delete_config } from './person.config'

export const person = new Elysia().group('/person', (app) =>
  app
    .get(
      '',
      async ({ query }) => {
    ...
    .get(
      ...
      async ({ params, set }) => {
        const found = await db
          .select()
          .from(table)
          .where(eq(table.personId, params.id))
    ...
    .post(
      '',
      async ({ body, set }) => {
        const record = { ...body, personId: crypto.randomUUID() }
    ...
    .patch(
      ...
      async ({ params, body, set }) => {
        ...
        await db.update(table).set(patch).where(eq(table.personId, params.id))
        set.status = 200
    ...
    .delete(
      ...
      async ({ params, set }) => {
        ...
        await db.delete(table).where(eq(table.personId, params.id))
        set.status = 202
