...
import { regions as table } from '../../db/schema'
import { get_config, get_by_id_config, post_config, patch_config, delete_config } from './region.config'

export const region = new Elysia().group('/region', (app) =>
  app
    .get(
      '',
      async ({ query }) => {
    ...
    .get(
      ...
      async ({ params, set }) => {
        const found = await db
          .select()
          .from(table)
          .where(eq(table.regionId, params.id))
    ...
    .post(
      '',
      async ({ body, set }) => {
        const record = { ...body, regionId: crypto.randomUUID() }
    ...
    .patch(
      ...
      async ({ params, body, set }) => {
        ...
        await db.update(table).set(patch).where(eq(table.regionId, params.id))
        set.status = 200
    ...
    .delete(
      ...
      async ({ params, set }) => {
        ...
        await db.delete(table).where(eq(table.regionId, params.id))
        set.status = 202
