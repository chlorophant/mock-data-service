...
import { get_config, get_by_id_config, post_config, patch_config, delete_config } from './user.config'
...
export const user = new Elysia().group('/user', (app) =>
  app
    ...
    .get(
      '/:id',
      async ({ params, set }) => {
        const found = await db.select().from(table).where(eq(table.userId, params.id)).orderBy(table.createdDate).all()
        if (!found.length) {
          set.status = 404
    ...
    .post(
      '',
      async ({ body, set }) => {
        const record = { ...body, userId: crypto.randomUUID() }
    ...
    .patch(
      ...
      async ({ params, body, set }) => {
        ...
        await db.update(table).set(patch).where(eq(table.userId, params.id))
        set.status = 200
    ...
    .delete(
      ...
      async ({ params, set }) => {
        ...
        await db.delete(table).where(eq(table.userId, params.id))
        set.status = 202
